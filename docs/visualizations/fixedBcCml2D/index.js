...
let topBoundary = new Array(size);
let rightBoundary = new Array(size);
let leftBoundary = new Array(size);
let bottomBoundary = new Array(size);
let boundaryPlot = null;

function setBoundaryValueAt(index, value) {
  topBoundary[index] = value;
  rightBoundary[index] = value;
  bottomBoundary[index] = value;
  leftBoundary[index] = value;
}

function renderBoundaryPlot() {
  if (!boundaryPlot) return;

  const values = topBoundary;
...
function refreshBoundaryValues() {
  for (let i = 0; i < size; i++) {
    // Keep all four edges synchronized to the same profile.
    setBoundaryValueAt(i, boundaryValueFromConfig(i, boundaryConfig));
  }
  renderBoundaryPlot();
}
...
function buildSharedBoundaryControl() {
  ...
  const bcModeSection = section.append("div")
    .style("display", "flex")
    .style("flex-direction", "column")
    .style("align-items", "center")
    .style("gap", "8px")
  ...
  const modeSelect = bcModeSection.append("select")
    .style("width", "170px")
  ...
  const paramRow = bcModeSection.append("div")
    .style("display", "grid")
...
function buildBoundaryPlotControl() {
  ...
  const dragBehavior = d3.drag()
    .on("start drag", (event, d) => {
      ...
      setBoundaryValueAt(d.index, newValue);
      renderBoundaryPlot();
      draw(lattice);
    });
...
const updateKernel = gpu.createKernel(function (lattice, alpha, epsilon, topBoundary, rightBoundary, bottomBoundary, leftBoundary) {
  const N = this.constants.size;
  const i = this.thread.y;
  const j = this.thread.x;

  // Boundary cells remain fixed (Dirichlet boundary conditions) - do not update them
  if (i === 0) return topBoundary[j];
  if (i === N - 1) return bottomBoundary[j];
  if (j === 0) return leftBoundary[i];
  if (j === N - 1) return rightBoundary[i];

  // Interior cells are updated based on coupling from all 4 neighbors
  const self = lattice[i][j];
  // Interior cells adjacent to boundaries read directly from boundary arrays
  const fromTop = (i === 1) ? topBoundary[j] : lattice[i - 1][j];
  const fromRight = (j === N - 2) ? rightBoundary[i] : lattice[i][j + 1];
  const fromBottom = (i === N - 2) ? bottomBoundary[j] : lattice[i + 1][j];
  const fromLeft = (j === 1) ? leftBoundary[i] : lattice[i][j - 1];

  const f_self = alpha * self * (1 - self);
  const f_top = alpha * fromTop * (1 - fromTop);
  const f_right = alpha * fromRight * (1 - fromRight);
  const f_bottom = alpha * fromBottom * (1 - fromBottom);
  const f_left = alpha * fromLeft * (1 - fromLeft);

  // 4-direction coupling: each interior site receives from all 4 neighbors.
  return (1 - epsilon) * f_self +
         (epsilon / 4.0) * (f_top + f_right + f_bottom + f_left);
})
.setOutput([size, size])
.setConstants({ size });

function initializeRandomLattice() {
  const next = new Array(size);
  for (let i = 0; i < size; i++) {
    next[i] = new Array(size);
    for (let j = 0; j < size; j++) {
      next[i][j] = Math.random() * 0.5 + 0.25;
    }
  }
  return next;
}
...
// Initialize lattice
let lattice = initializeRandomLattice();
refreshBoundaryValues();
...
function step() {
  ...
  for (let s = 0; s < stepsPerFrame; s++) {
    lattice = updateKernel(lattice, alpha, epsilon, topBoundary, rightBoundary, bottomBoundary, leftBoundary);
  }
  draw(lattice);
...
resetBtn.on("click", () => {
  refreshBoundaryValues();
  lattice = initializeRandomLattice();
  draw(lattice);
});
