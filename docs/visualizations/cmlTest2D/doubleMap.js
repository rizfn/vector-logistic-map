...
const updateKernel = gpu.createKernel(function (lattice, alpha, epsilon) {
  ...
  // Double logistic map: f(f(x)) = α² x(1-x)(1 - αx(1-x))
  const f_self = alpha * alpha * self * (1 - self) * (1 - alpha * self * (1 - self));
  const f_up = alpha * alpha * up * (1 - up) * (1 - alpha * up * (1 - up));
  const f_down = alpha * alpha * down * (1 - down) * (1 - alpha * down * (1 - down));
  const f_left = alpha * alpha * left * (1 - left) * (1 - alpha * left * (1 - left));
  const f_right = alpha * alpha * right * (1 - right) * (1 - alpha * right * (1 - right));
